...
class HashOutput:
    """
    A utility class for displaying hash-related information in a formatted manner.
    """
    ...
    def display_file_hash(file_path: Path, algorithm: str, file_hash: str):
        ...
        HashOutput.console.print(table)
    ...
    def display_text_hash(text: str, algorithm: str, text_hash: str):
        ...
        """
        HashOutput.console.print(f"Text: {text}")
        HashOutput.console.print(f"Algorithm: {algorithm}")
        HashOutput.console.print(f"Hash: {text_hash}")

    @staticmethod
    def display_successful_verification(file_path: Path, algorithm: str, hash_value: str):
        """
    ...
    @staticmethod
    def display_failed_verification(file_path: Path, algorithm: str, known_hash: str, calculated_hash: str):
        """
    ...
    @staticmethod
    def display_success_comparison(file1: Path, file2: Path, algorithm: str, hash_value: str):
        """
    ...
    @staticmethod
    def display_failure_comparison(file1: Path, file2: Path, algorithm: str):
        """
    ...
    @staticmethod
    def display_manifest(manifest_path: Path, algorithm: str, directory_hash: str, file_count: int):
        """
    ...
    @staticmethod
    def display_successful_dir_comparison(directory1: Path, directory2: Path, algorithm: str, directory_hash: str):
        """
    ...
    @staticmethod
    def display_failed_dir_comparison(directory1: Path, directory2: Path, algorithm: str, comparison, verbose: bool = False):
        """
        ...
        if comparison.new_files:
            ...
            for file in comparison.new_files:
                table.add_row(
                    file.path,
                    f"{file.size} bytes",
                    HashOutput.shorten_hash(file.digest)
                )
    ...
    Hash: {HashOutput.shorten_hash(edited_file.old_file.digest)}

    [green]NEW[/green]
    Size: {edited_file.new_file.size} bytes
    Hash: {HashOutput.shorten_hash(edited_file.new_file.digest)}
                        """,
                        border_style="yellow"
                    )
                )
